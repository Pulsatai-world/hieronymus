...
const TX = {
  ...
  summaryLine:  { es: 'Se pudo entrar al sitio web y {p:se analizó|se analizaron} {p} {p:página|páginas} con {c} {c:revisión|revisiones}. {b}', en: 'The site was reachable and {p} {p:page|pages} could be analysed across {c} {c:check|checks}. {b}' },
  ...
  missingFix:   { es: 'Decide primero si este sitio web debe crecer a varias páginas y, si es así, crea las que falten y enlázalas desde la navegación principal o el pie.', en: 'Decide first whether this site should grow into several pages and, if so, create the missing ones and link them from the main navigation or the footer.' },
