...
3. **Prompt generation** — triggered from the per-customer page, handled server-side by `netlify/functions/generate-prompts-background.js` (a Background Function; the earlier `generate-prompts.js` name is gone) billed to **that customer's own Claude key** (`customer-keys.js` → `claude`), read server-to-server and never entered in or returned to the browser. There is deliberately no platform-wide key: every engine call in the platform belongs to one customer, and so does its cost and rate limit. A customer with no Claude key configured is refused before a single call goes out. The modal asks for **count and language(s) only** — engine choice moved to Run Audit, because which engines to ask is a property of a run, not of a prompt set.
...
## Local development
Two commands, because two-factor makes a local instance unusable without seeded accounts — the first
staff account has nobody to create it, and every account after that needs an authenticator.

```
npm run dev              netlify dev on :8888
npm run seed             create a staff admin + two demo customers, and enrol them
npm run seed -- --reset  the same, after wiping .netlify/blobs-serve (LOCAL data only)
npm run code             live 6-digit codes for every seeded account — no phone needed
npm test                 every suite
```

`scripts/seed-local.mjs` drives the real HTTP API rather than writing to storage, so what it creates
is what a real sign-up creates; if a rule changes it breaks the same way a person would. Usernames
and passwords for customers are **generated by the endpoint**, not chosen — the seed uses what it
gets back. Everything lands in `.local-dev-accounts.json` (gitignored).

Local Blobs live on disk under `.netlify/blobs-serve` and survive restarts, so a store seeded weeks
ago is still there; that is why the bootstrap refuses and points at `--reset`.

**Background functions always answer 202.** `/api/run-audit` and `/api/generate-prompts` are Netlify
Background Functions: the HTTP response is Netlify's acknowledgement, sent before the function runs,
so a 202 says nothing about whether the work was authorised or done. To check one was refused, look
for the absence of a job record — not at the status code.
...
- **There is no shared platform API key.** Prompt generation and audit runs both spend the customer's own Claude key. `ANTHROPIC_API_KEY` used to back generation and is gone — generation was the last piece whose spend, rate limit and blast radius were pooled across every customer, and it meant a customer with correctly configured keys could still be told an API key was invalid, with nothing on their record that changing would fix.
